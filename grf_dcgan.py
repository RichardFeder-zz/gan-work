...
class GAN_optimization():
	...
	def __init__(self, optimizerD, optimizerG, netD, netG, optimizer_info=None):
        ...
        def discriminator_step(self, real_cpu, real_code=None):
                self.netD.zero_grad()
                self.netG.zero_grad()
                ...
                if real_code is not None:
                        output, guess_code = self.netD(real_cpu)
                        errD_code = opt.lam_info * mutualinfo_loss(guess_code, real_code)
                        errD_code.backward(retain_graph=True)
                else:
        ...
        def single_train_step(self, real_cpu, real_code=None):
                errD, D_x, D_G_z1 = self.discriminator_step(real_cpu, real_code)
                for i in xrange(5):
	...
	if opt.trainSize > 0:
		...
		for local_batch in training_generator:
			...
			lossGs.append(errG.item())
                ...
                if opt.code_dim > 0:
                        alphas = np.random.choice(np.linspace(-0.5, 0.5, 11), opt.batchSize)
                        #alphas = np.random.uniform(opt.alpha-0.5, opt.alpha+0.5, opt.batchSize)
                        dat, amps = gaussian_random_field(opt.batchSize, opt.alpha+alphas, opt.imageSize)
                        real_code = alphas
                        real_code = torch.from_numpy(real_code).to(device).float()
                else:
                        alphas = np.random.choice(np.linspace(-1, 1, 11), opt.batchSize) 
                        #alphas = np.random.uniform(opt.alpha-0.5, opt.alpha+0.5, opt.batchSize)
                        dat, amps = gaussian_random_field(opt.batchSize, opt.alpha+alphas, opt.imageSize)
		...
		real_cpu = torch.from_numpy(dat).to(device)
                if opt.code_dim > 0:
                        #real_code = torch.from_numpy(alphas).to(device)
                        errD, errG, errI, D_x, D_G_z1, D_G_z2 = ganopt.single_train_step(real_cpu, real_code=real_code)
        ...
        if opt.n_epochs > 1000:
            ...
            if i % int(opt.n_epochs/10) == 1:
                ...
                print('saving intermediate models for epoch', i)
